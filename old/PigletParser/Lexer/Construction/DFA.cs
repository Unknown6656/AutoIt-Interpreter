        ...
        public void Minimize()
        {
            ...
            do
            {
                ...
                distinctStatePairs((p, q) =>
                {
                    if (distinct[p, q] == -1)
                    {
                        State targetState(State state, CharRange c) => targetDict.TryGetValue(state, out Dictionary<CharRange, State> charDict) && charDict.TryGetValue(c, out State toState) ? toState : null;
            ...
            foreach (ISet<State> mergeSet in mergeSets)
            {
                ...
                for (int i = 1; i < stateList.Count; ++i)
                {
                    ...
                    foreach (Transition<State> transition in Transitions.Where(f => f.To == toRemove).ToList())
                    {
                    ...
                    foreach (Transition<State> transition in Transitions.Where(f => f.From == toRemove).ToList())
                    {
